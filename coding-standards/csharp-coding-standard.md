# C# 코딩 표준

## 1. 기본 원칙

이 코딩 표준은 가독성과 일관성을 최우선 가치로 삼는다.  
코드는 단순히 동작하는 것을 넘어, 읽는 사람이 빠르고 정확하게 의도를 이해할 수 있어야 한다.
  
- 가독성을 최우선으로 한다.
  - 코드는 주석 없이도 최대한 의도를 이해할 수 있어야 한다.
  - 복잡한 로직은 트릭보다 명확한 표현을 우선한다.
- 특별한 이유가 없는 한 IDE의 자동 서식 규칙을 따른다.
  - 비주얼 스튜디오의 `Ctrl + K + D` 기능을 기본으로 사용한다.
  - 개인 취향보다 팀 전체의 일관성을 우선한다.
- 이름은 역할과 의도를 드러내야 한다.
  - 축약어, 애매한 약어, 임의의 줄임말보다 명확한 이름을 사용한다.
- 타입은 의미를 표현해야 한다.
  - 메서드의 매개변수와 반환값은 가능한 한 강타입으로 설계한다.
  - 문자열, 숫자, 불리언으로 의미를 암묵적으로 전달하는 설계를 지양한다.

이 코딩 표준은 아래의 코딩 표준들을 참고하되, 실제 개발 환경에서의 가독성과 유지보수성을 고려하여 일부 항목을 현실적으로 조정하였다.

- [언리얼 엔진 4 코딩 표준](https://docs.unrealengine.com/latest/INT/Programming/Development/CodingStandard/)
- [둠 3 코드 스타일과 규칙](ftp://ftp.idsoftware.com/idstuff/doom3/source/codestyleconventions.doc)
- [IDesign C# 코딩 표준](http://www.idesign.net/downloads/getdownload/1985)

## 2. 파일, 네임스페이스, 클래스 구조

### 2.1. 네임스페이스 규칙

- 네임스페이스 이름은 파스칼 표기법을 따른다.
- 파일 범위 namespace 선언(C# 10.0)을 사용한다.

```cs
namespace Company.Product.Billing;
```

### 2.2. 클래스와 파일 구조

- 클래스는 각각 독립된 소스 파일에 있어야 한다.
  - 단, 작은 클래스 몇 개를 한 파일 안에 같이 넣어두는 것이 상식적일 경우 예외를 허용한다.
- 파일 이름은 대소문자까지 포함해서 반드시 클래스 이름과 일치해야 한다.

```cs
public class InvoiceService
{
}
```

```text
InvoiceService.cs
```

### 2.3. partial 클래스 파일 이름 규칙

- 여러 파일이 하나의 클래스를 이룰 때(즉, partial 클래스), 파일 이름은 클래스 이름으로 시작하고 그 뒤에 마침표와 세부 항목 이름을 붙인다.

```cs
public partial class CharacterController
{
}
```

```text
CharacterController.Input.cs
CharacterController.Movement.cs
CharacterController.Animation.cs
```

## 3. 네이밍 규칙

### 3.1. 클래스, 인터페이스, 구조체, 열거형

- 클래스와 구조체의 이름은 파스칼 표기법을 따른다.
- 인터페이스를 선언할 때는 앞에 `I`를 붙인다.
- 열거형을 선언할 때는 앞에 `E`를 붙인다.
- 구조체를 선언할 때는 앞에 `S`를 붙인다.
  - 단, `readonly struct`일 때는 그렇지 않다.
- 비트 플래그 열거형은 이름 뒤에 `Flags`를 붙인다.

```cs
public class OrderService
{
}

public interface IUserRepository
{
}

public enum EPaymentStatus
{
    Pending,
    Approved,
    Rejected
}

public struct SWindowSize
{
}

[Flags]
public enum EPermissionFlags
{
    None = 0,
    Read = 1 << 0,
    Write = 1 << 1,
    Delete = 1 << 2,
}
```

### 3.2. 메서드 이름 규칙

- 메서드 이름은 반드시 동사 또는 동사구로 시작한다.
- 가능하면 `동사 + 목적어` 형태로 작성한다.
- 반환값이 있는 메서드는 무엇을 반환하는지가 이름에 드러나야 한다.
- `bool` 값을 반환하는 메서드는 `Is`, `Has`, `Can`, `Should`를 우선 사용한다.
  - 그러는 것이 부자연스러운 경우에는 상태를 나타내는 다른 3인칭 단수형 동사를 사용한다.
- 재귀 함수는 이름 끝에 `Recursive`를 붙인다.
- public 메서드 이름은 파스칼 표기법을 따른다.
- public 메서드가 아닌 경우 카멜 표기법을 따른다.

```cs
public Invoice GetInvoiceByID(int invoiceID)
{
    // 함수 구현부...
}

public decimal CalculateMonthlyRevenue()
{
    // 함수 구현부...
}

public bool IsExpired(Coupon coupon);
public bool HasPermission(User user);
public bool CanRetry(Payment payment);
public bool ShouldArchive(Order order);
public bool Exists(Customer customer);

public void TraverseDirectoryRecursive(string directoryPath);

private Invoice getInvoiceByID(int invoiceID)
{
    // 함수 구현부...
}
```

### 3.3. 변수 이름 규칙

- 지역 변수와 함수의 매개변수 이름은 카멜 표기법을 따른다.
- 변수명은 명사 또는 명사구를 사용한다.
- 값의 의미, 역할, 상태, 개수, 위치가 드러나야 한다.
- 의미 없는 축약어, 임의의 약어, 단일 문자 변수명은 사용하지 않는다.
  - 단, 짧은 반복문 인덱스처럼 관례적으로 명확한 경우는 예외로 한다.
- 단순 반복문 변수가 아닌 경우 `i`, `e` 같은 변수명 대신 `index`, `employee`처럼 저장된 데이터를 바로 알 수 있는 이름을 사용한다.
- 컬렉션은 복수형으로 작성한다.
- 키-값 대응 컬렉션은 가능하면 `xxxByYyy` 형식을 사용한다.
- `bool` 변수는 `is`, `has`, `can`, `should`를 우선 사용한다.

```cs
public void SendInvoiceEmail(int invoiceID, string recipientEmail)
{
    int retryCount;
    int currentIndex;
    string filePath;

    List<Order> pendingOrders;
    Dictionary<int, User> usersByID;

    bool isConnected;
    bool hasDiscount;
    bool canRetry;
    bool shouldUpdateCache;
}
```

### 3.4. 멤버 변수 이름 규칙

- `private` 인스턴스 멤버 변수는 앞에 `m`을 붙이고 파스칼 표기법을 따른다.
- `static` 멤버 변수는 앞에 `s`를 붙이고 파스칼 표기법을 따른다.
- 접두어 뒤에는 필드의 원래 이름을 대문자로 시작하여 이어 붙인다.
- `bool` 멤버 변수는 `mIs`, `mHas`, `mCan`, `mShould` 형식을 우선 사용한다.

```cs
public class PaymentProcessor
{
    private int mRetryCount;
    private string mGatewayName;
    private bool mIsConnected;
    private bool mHasPendingRequest;

    private static string sDefaultCurrencyCode;
    private static int sMaxRetryCount;
}
```

### 3.5. 프로퍼티 이름 규칙

- `bool` 프로퍼티는 앞에 `Is`, `Has`, `Can`, `Should` 중 하나를 붙인다.

```cs
public bool IsClosed { get; private set; }
public bool HasUnreadMessage { get; private set; }
public bool CanExport { get; private set; }
public bool ShouldRefresh { get; private set; }
```

### 3.6. 상수와 줄임말 규칙

- 상수의 이름은 모두 대문자로 하되 밑줄로 각 단어를 분리한다.
- 상수로 사용하는 개체형 변수에는 `static readonly`를 사용한다.
- 상수로 사용하는 `static readonly` 변수는 모두 대문자로 하되 밑줄로 각 단어를 분리한다.
- 뒤에 추가적인 단어가 오지 않는 경우 줄임말은 모두 대문자로 표기한다.

```cs
const int MAX_RETRY_COUNT = 3;
const string DEFAULT_COUNTRY_CODE = "KR";

public static readonly CurrencyInfo DEFAULT_CURRENCY = new CurrencyInfo("KRW");

public int OrderID { get; private set; }
public int HttpCode { get; private set; }
public string ApiKey { get; private set; }
```

## 4. 접근 제어 및 캡슐화

### 4.1. 프로퍼티 사용 규칙

- getter와 setter 대신 프로퍼티를 사용한다.

틀린 방식:

```cs
public class UserProfile
{
    private string mDisplayName;

    public string GetDisplayName();
    public string SetDisplayName(string displayName);
}
```

올바른 방식:

```cs
public class UserProfile
{
    public string DisplayName { get; set; }
}
```

### 4.2. 읽기 전용 값 규칙

- 초기화 후 값이 변하지 않는 변수는 `readonly`로 선언한다.
- 프로퍼티에 `private init`(C# 9.0)을 최대한 사용한다.

```cs
public class Account
{
    private readonly string mPasswordHash;
    private readonly IUserRepository mUserRepository;

    public string EmailAddress { get; private init; }

    public Account(string passwordHash, IUserRepository userRepository)
    {
        mPasswordHash = passwordHash;
        mUserRepository = userRepository;
    }
}
```

## 5. 변수 및 메서드 설계 규칙

### 5.1. 지역 변수 선언 규칙

- 지역 변수를 선언할 때는 그 지역 변수를 사용하는 코드와 최대한 가까운 위치에 선언한다.

```cs
public decimal GetDiscountedPrice(decimal originalPrice, decimal discountRate)
{
    decimal discountedPrice = originalPrice * (1.0m - discountRate);
    return discountedPrice;
}
```

### 5.2. 메서드 오버로딩 규칙

- 매개변수 자료형이 범용적인 경우 함수 오버로딩을 피한다.

올바른 방식:

```cs
public AnimationClip GetAnimationByIndex(int index);
public AnimationClip GetAnimationByName(string animationName);
```

틀린 방식:

```cs
public AnimationClip GetAnimation(int index);
public AnimationClip GetAnimation(string animationName);
```

### 5.3. 디폴트 매개변수 규칙

- 디폴트 매개 변수 대신 함수 오버로딩을 선호한다.
- 디폴트 매개 변수를 사용하는 경우 `null`, `false`, `0` 같이 비트 패턴이 0인 값을 사용한다.

```cs
public void SaveReport()
{
    SaveReport(false);
}

public void SaveReport(bool shouldOverwrite)
{
    // 함수 구현부...
}
```

### 5.4. 변수 가리기 금지

- 변수 가리기(variable shadowing)는 허용하지 않는다.
- 외부 변수가 동일한 이름을 사용 중이라면 내부 변수에는 다른 이름을 사용한다.

틀린 방식:

```cs
public class InvoiceService
{
    private int mRetryCount;

    public void ProcessInvoices(int retryCount)
    {
        int totalCount = 10;
    }
}
```

올바른 방식:

```cs
public class InvoiceService
{
    private int mRetryCount;

    public void ProcessInvoices(int maxRetryCount)
    {
        int totalCount = 10;

        for (int i = 0; i < totalCount; ++i)
        {
            // i를 사용
        }
    }
}
```

### 5.5. 강타입 매개변수 설계 규칙

- 매개변수는 가능한 한 강타입으로 받는다.
- 의미를 암묵적으로 담은 `string`, `int`, `long`, `bool`의 직접 사용을 지양한다.
- 정해진 선택지는 `enum`으로 표현한다.
- 도메인 의미가 있는 값은 전용 타입으로 감싸는 것을 우선 검토한다.
- 관련된 여러 매개변수가 함께 이동하면 별도의 객체로 묶는다.
- 의미가 다른 동일 타입 매개변수를 나란히 받는 설계를 피한다.
- `bool` 매개변수는 호출부 가독성을 떨어뜨리므로 가급적 사용하지 않는다.

#### enum 사용 예

```cs
public enum ESortOrder
{
    Ascending,
    Descending
}

public void SortInvoices(ESortOrder sortOrder)
{
    // 함수 구현부...
}
```

#### 전용 타입 사용 예

```cs
public readonly record struct UserID(Guid Value);
public readonly record struct EmailAddress(string Value);
public readonly record struct ProductCode(string Value);

public User GetUserByID(UserID userID)
{
    // 함수 구현부...
}

public void SendWelcomeEmail(EmailAddress emailAddress)
{
    // 함수 구현부...
}
```

#### parameter object 사용 예

```cs
public sealed class OrderSearchCriteria
{
    public string Keyword { get; init; }
    public EOrderStatus Status { get; init; }
    public int PageNumber { get; init; }
    public int PageSize { get; init; }
}

public List<Order> GetOrders(OrderSearchCriteria criteria)
{
    // 함수 구현부...
}
```

### 5.6. 외부 입력 검증 규칙

- 외부 입력은 시스템 경계에서 검증한다.
- 검증 실패 시 가능한 빨리 반환한다.
- 내부 메서드는 이미 검증된 값을 받는다고 가정한다.

```cs
private readonly IUserRepository mUserRepository;

public User GetUserByEmailOrNull(string emailOrNull)
{
    if (string.IsNullOrWhiteSpace(emailOrNull))
    {
        return null;
    }

    EmailAddress emailAddress = new EmailAddress(emailOrNull);
    return GetUserByEmail(emailAddress);
}

private User GetUserByEmail(EmailAddress emailAddress)
{
    return mUserRepository.GetByEmail(emailAddress);
}
```

### 5.7. null 사용 규칙

- 함수의 매개변수로 `null`을 허용하지 않는 것을 추구한다.
  - 특히 public 함수일 경우 더욱 그러하다.
- `null` 값을 허용하는 매개변수를 사용할 경우 변수명 뒤에 `OrNull`을 붙인다.
- 함수에서 `null`을 반환하지 않는 것을 추구한다.
  - 특히 public 함수일 경우 더욱 그러하다.
  - 그러나 때로는 예외를 던지는 것을 방지하기 위해 `null` 반환이 필요할 수도 있다.
- 함수에서 `null`을 반환할 때는 함수 이름 뒤에 `OrNull`을 붙인다.

```cs
public AnimationClip GetAnimationOrNull(string animationNameOrNull)
{
    // 함수 구현부...
}

public string GetDisplayNameOrNull();
```

## 6. 제어문, assert, 예외 처리 규칙

### 6.1. switch 문 규칙

- `switch` 문에 언제나 `default:` 케이스를 넣는다.
- `switch` 문에서 `default:` 케이스가 절대 실행될 일이 없는 경우, `default:` 안에 `Debug.Fail()`을 추가한다.
- 위 원칙의 예외로, `enum` 형을 `switch` 문에서 처리할 때 실수로 처리하지 않은 `enum` 값을 찾기 위해 `default:` 케이스에서 예외를 던지는 것은 허용한다.

```cs
switch (menuIndex)
{
    case 0:
        OpenHomePage();
        break;
    default:
        break;
}
```

```cs
switch (messageType)
{
    case 1:
        SendTextMessage();
        break;
    default:
        Debug.Fail("unknown message type");
        break;
}
```

```cs
switch (accountType)
{
    case AccountType.Personal:
        return personalDiscountRate;
    case AccountType.Business:
        return businessDiscountRate;
    default:
        throw new NotImplementedException($"unhandled switch case: {accountType}");
}
```

### 6.2. assert 규칙

- 코드를 작성하면서 세운 모든 가정에 `Debug.Assert()`를 넣는다.
- 특정 조건이 반드시 충족되어야 한다고 가정하고 짠 코드 모든 곳에 `assert`를 사용한다.
- `assert`는 복구 불가능한 조건이다.

```cs
public decimal Divide(decimal dividend, decimal divisor)
{
    Debug.Assert(divisor != 0.0m);

    return dividend / divisor;
}
```

## 7. 비동기, 컬렉션, 언어 기능 사용 규칙

### 7.1. 컬렉션 사용 규칙

- 언제나 `System.Collections`에 들어있는 컨테이너 대신에 `System.Collections.Generic`에 들어있는 컨테이너를 사용한다.
- 순수 배열을 사용하는 것도 괜찮다.

```cs
List<Order> completedOrders = new List<Order>();
Dictionary<int, User> usersByID = new Dictionary<int, User>();
string[] fileNames = new string[10];
```

### 7.2. var 사용 규칙

- `var` 키워드를 사용하지 않는다.
  - 단, 데이터형이 중요하지 않은 경우에는 예외를 허용한다.
  - `IEnumerable`에 `var`를 사용하거나 익명 타입을 사용할 때가 좋은 예이다.

기본 원칙:

```cs
Order order = new Order();
List<User> users = new List<User>();
Dictionary<int, Product> productsByID = new Dictionary<int, Product>();
```

허용 가능한 예외:

```cs
var groupedOrders = orders.GroupBy(order => order.CustomerID);

var summary = new
{
    TotalCount = orders.Count,
    TotalAmount = orders.Sum(order => order.Price),
};
```

### 7.3. 싱글턴과 static 클래스

- 싱글턴 패턴 대신에 정적(`static`) 클래스를 사용한다.

```cs
public static class DateTimeUtility
{
    public static DateTime GetStartOfDay(DateTime dateTime)
    {
        return new DateTime(dateTime.Year, dateTime.Month, dateTime.Day);
    }
}
```

### 7.4. async / await 규칙

- `async void` 대신에 `async Task`를 사용한다.
  - `async void`가 허용되는 유일한 곳은 이벤트 핸들러이다.
- `async` 메서드명에 `Async` 접미사를 붙이지 않는다.

```cs
public async Task SaveOrder(CancellationToken cancellationToken)
{
    await mOrderRepository.Save(cancellationToken);
}

private async void xButtonSave_Click(object sender, EventArgs e)
{
    await SaveOrder(CancellationToken.None);
}
```

### 7.5. 최신 문법 사용 규칙

- 인라인 람다는 한 줄짜리 짧은 코드만 담을 수 있다.
- 개체 초기자(object initializer)를 사용하지 않으려고 노력한다.
  - 단, `required` 한정자(C# 11.0)와 초기화 전용 setter(C# 9.0)와 같이 사용할 때는 괜찮다.
- 함수에 전달하는 `out` 매개변수는 별도의 라인에 선언한다.
  - 인자 목록 안에서 선언하지 않는다.
- null 병합 연산자(C# 7.0)의 사용을 금한다.
- `using` 선언(C# 8.0)의 사용을 금한다.
  - 대신 `using` 문을 사용한다.
- `new` 키워드 뒤에 반드시 명시적으로 자료형을 적어준다.
  - 즉, C# 9.0의 `new()` 사용을 금지한다.
  - 단, 함수 내부에서 익명 타입을 사용할 때는 허용한다.
- 범용적인 자료형을 강타입(strong type)으로 만들 때는 `readonly record struct`(C# 10.0)를 사용한다.

```cs
orders.ForEach(order => PrintOrder(order));

using (StreamReader reader = new StreamReader(filePath))
{
    string fileContent = reader.ReadToEnd();
}

bool isParsed = int.TryParse(text, out int value); // 금지
```

올바른 방식:

```cs
int value;
bool isParsed = int.TryParse(text, out value);
```

```cs
User user = new User();
```

```cs
public readonly record struct OrderID(Guid Value);
```

## 8. 클래스 내부 구성 규칙

- 클래스 안에서 멤버 변수와 메서드의 등장 순서는 다음을 따른다.
  1. 멤버 변수
  2. 프로퍼티
     - 단, 프로퍼티와 대응하는 private 멤버변수는 프로퍼티 바로 위에 적는다.
  3. 생성자
  4. 메서드
     - public -> private 순서로 배치한다.
- 클래스 안에서 연관 있는 메서드끼리 그룹을 짓는다.
- 멤버 변수도 연관 있는 것끼리 그룹을 짓는다.

```cs
public class UserService
{
    private readonly IUserRepository mUserRepository;
    private readonly IEmailSender mEmailSender;
    private bool mHasPendingRequest;

    public int UserCount
    {
        get;
        private set;
    }

    public UserService(IUserRepository userRepository, IEmailSender emailSender)
    {
        mUserRepository = userRepository;
        mEmailSender = emailSender;
    }

    public User GetUserByID(UserID userID)
    {
        return mUserRepository.GetByID(userID);
    }

    public void SendWelcomeEmail(UserID userID)
    {
        User user = GetUserByID(userID);
        sendWelcomeEmailInternal(user);
    }

    private void sendWelcomeEmailInternal(User user)
    {
        mEmailSender.Send(user.EmailAddress);
    }
}
```

## 9. 소스 코드 포맷팅

### 9.1. 들여쓰기와 탭

- 탭(tab)은 비주얼 스튜디오 기본값을 사용하며, 비주얼 스튜디오를 사용하지 않을 시 띄어쓰기 4칸을 탭으로 사용한다.

### 9.2. 중괄호 규칙

- 중괄호(`{`)를 열 때는 언제나 새로운 줄에 연다.
- 중괄호 안에 코드가 한 줄만 있더라도 반드시 중괄호를 사용한다.

```cs
if (shouldCloseWindow)
{
    return;
}
```

### 9.3. 변수 선언 규칙

- 한 줄에 변수 하나만 선언한다.

틀린 방식:

```cs
int counter = 0, index = 0;
```

올바른 방식:

```cs
int counter = 0;
int index = 0;
```

## 10. 프로젝트 설정 관련

- 배포(release) 빌드에서 컴파일러 경고(warning)를 오류(error)로 처리하게 설정한다.
- implicit global using(C# 10.0)을 사용하지 않는다.
- nullable context를 사용한다.

## 11. 특정 프레임워크 및 패턴용 가이드라인

### 11.1. 자동 직렬화 (예: `System.Text.Json`)

- 자동 직렬화에 사용할 데이터는 `class`로 정의한다.
- 자동 직렬화용 `class` 파일에는 특정 직렬화 라이브러리 전용 attribute를 넣지 않는다.
- 자동 직렬화용 `class`의 데이터는 언제나 `public` 자동 프로퍼티를 사용한다.
  - 프로퍼티와 멤버 필드의 1:1 대칭 관계를 유지한다.
- 자동 직렬화용 `class`에 읽기 전용 프로퍼티가 필요한 경우, 그 대신 `public` 메서드를 만든다.
- 자동 직렬화용 `class`의 `public` 생성자는 언제나 하나만 두며, 이 생성자는 매개변수를 받지 않는다.
- 자동 직렬화 함수(예: `JsonSerializer.Serialize<>()`)를 직접 호출하지 않는다.
  - wrapper 함수를 만들어 직렬화 함수에 넣을 수 있는 매개변수의 자료형을 통제한다.

```cs
public class UserResponse
{
    public string DisplayName { get; set; }
    public string EmailAddress { get; set; }

    public UserResponse()
    {
    }

    public bool HasEmailAddress()
    {
        return string.IsNullOrWhiteSpace(EmailAddress) == false;
    }
}
```

### 11.2. XAML 컨트롤

- 컨트롤의 이름(예: `x:Name`)은 정말 필요한 경우에만 만든다.
- 컨트롤의 변수명 앞에는 `x`를 붙인다.
- 컨트롤의 변수명 앞에는 컨트롤 종류도 붙인다.

```cs
xLabelUserName
xButtonSubmit
xTextBoxEmailAddress
xGridLayoutRoot
```

### 11.3. ASP.NET Core

- REST API의 요청 바디(request body)용 DTO(Data Transfer Object)를 만들 때, 각 값형 프로퍼티를 `nullable`로 만들어 모델 유효성 검증(model validation)이 자동으로 이뤄지게 한다.

```cs
public class UpdateUserRequest
{
    [Required]
    public Guid? UserID { get; set; }

    [Required]
    public string? DisplayName { get; set; }
}
```

- 컨트롤러 메서드에서 요청에서 처음으로 할 일은 입력값 검증이다.
- 이 검증을 통과하면 모든 입력값은 유효한 것으로 간주한다.
  - 따라서 `[Required]` 애트리뷰트가 달린 `nullable` 속성은 그 후부터는 모두 `null`이 아니다.
- 위 원칙과는 달리 `[RouteParam]` 애트리뷰트가 달린 변수는 `?`를 가지지 않는다.

```cs
public bool GetUser([RouteParam] Guid userID)
```
