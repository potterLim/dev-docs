...
- enum은 PascalCase를 사용하며, 이름 앞에 `E` 접두어를 사용한다.
...
- boolean 멤버 변수는 `mIs`, `mHas`, `mCan`, `mShould` 형식을 우선 사용한다.
...
- `getter`는 정말 읽기 접근이 필요할 때만 제공한다.
...
- 지역 변수 이름은 같은 범위 안에서 역할이 중복되지 않아야 한다.
...
- 전용 타입은 가능한 한 불변으로 설계하는 것을 우선한다.
- 관련된 여러 매개변수는 별도의 객체로 묶는 것을 우선 검토한다.
...
public void sortUsers(ESortOrder sortOrder) {
...
public final class OrderSearchCriteria {
    ...
    public OrderSearchCriteria(SearchKeyword keyword, EOrderStatus status, int pageNumber, int pageSize) {
        mKeyword = keyword;
        mStatus = status;
...
```java
public void saveUser(boolean shouldOverwrite);
```
...
public void saveUser(ESaveMode saveMode);
```
...
- `assert`는 논리적으로 절대 발생해서는 안 되는 상황을 검증할 때 사용한다.
- 외부 입력 검증이나 복구 가능한 오류 처리에는 `assert`를 사용하지 않는다.
- 반드시 괄호로 감싸서 작성한다.
